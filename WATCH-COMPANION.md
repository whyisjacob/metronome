...
The watch shows a large beat number, meter, Start/Stop, and ± tempo. For a song, tempo controls adjust its master percentage. Meter and vibration/spoken output live in Settings. Changing tempo/meter stops playback so the next Start uses the synchronized settings. Phone edits received during watch playback are applied after Stop.
